...
def k_time(product_time: str):
    ...
    if get_days_until_date(product_time) == 1 or get_days_until_date(product_time) == 0:
        return 1.0
...
def min_price(products_info:list[dict]) -> int:
    k = int(min(products_info, key=lambda x: int(x["Цена"].replace('\u2009', '')))['Цена'].replace('\u2009', ''))
    return k

def max_price(products_info:list[dict]) -> int:
    k = int(max(products_info, key=lambda x: int(x["Цена"].replace('\u2009', '')))['Цена'].replace('\u2009', ''))
    return k

def average_value(products_info:list[dict]) -> float:
    k = 0
    for i in products_info:
        k += int(i['Цена'].replace('\u2009',''))
...
def sorting_products(products: list[dict]) -> list:
    ...
    for item in products:
        try:
            rating = k_rating(float(item['Звёзды']))
        except:
            rating = 0
        try:
        ...
        price = k_price(min_, average,max_, int(item['Цена'].replace('\u2009','')))
