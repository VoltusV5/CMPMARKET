...
def sort(products):
    ...
    for i in range(4):
        s.append(onion[i][1])
    with open("parser/BeautifulOzonProducts.json", 'w', encoding="UTF-8") as file:
...
def colleсt_product_info(html:str ='') -> dict:
    ...
    all = soup.find_all('div', class_="xi4_23")
    s = []

    for link in all:
        product_info = {}

        product_info['Название'] = product_info.setdefault('Название', link.find('div', class_='xi5_23').find('span', class_="tsBody500Medium").text)
        product_info['Цена'] = product_info.setdefault('Цена', link.find('div',class_='c3024-a0').find_all('span')[1].text)
        product_info['Ссылка'] = product_info.setdefault('Ссылка', 'https://ozon.ru' + link.find('div', class_='xi5_23').find('a')['href'])
        product_info['Фото'] = product_info.setdefault('Фото', link.find('div', class_='i7y_23').find('img')['src'])
        try:
            product_info['Звёзды'] = product_info.setdefault('Звёзды', link.find('div', class_='xi5_23').find('div', class_='ix9_23').find('span').find('span').text)
        except:
            product_info['Звёзды'] = '1'
        try:
            product_info['Оценки'] = product_info.setdefault('Оценки',link.find('div', class_='xi5_23').find('div', class_='ix9_23').find_all('span')[-1].find('span').text)
        except:
            product_info['Оценки'] = '1'
        try:
            product_info['Время'] = product_info.setdefault('Время', link.find('div', class_='b2121-a1').find('div').text)
        except:
            product_info['Время'] = 'Завтра'
...
def get_products_links(item_name:str = 'Очки') -> None:
